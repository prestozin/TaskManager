...
public class TaskService : ITaskService
{
    ...
    public async Task<ResultDto<PagedResultDto<TaskResponseDto>>> GetAllTasks(Guid userId, PagedParamsDto pagedParams)
    {
        ...
        List<TaskResponseDto> taskDtos = tasks.Adapt<List<TaskResponseDto>>();
        
        if (!taskDtos.Any())
            return ResultDto<PagedResultDto<TaskResponseDto>>.Failure(string.Format("Nenhuma tarefa encontrada"));

        PagedResultDto<TaskResponseDto> pagedResult = new PagedResultDto<TaskResponseDto>(taskDtos, pagedParams.PageNumber, pagedParams.PageSize, totalCount);
